...
│ Cliente│ ──────────────────────────────▶│  Controller │ ──duplicados────▶ │ Model User    │
└────────┘   { firstName, email, pass }   └────────────┘                   │ (hook bcrypt)  │
     ▲                                           │                          └──────┬────────┘
...
LOGIN (próximo paso)
┌────────┐  POST /api/v1/auth/login   ┌────────────┐  busca user +   ┌──────────────┐
│ Cliente│ ──────────────────────────▶│ Controller │ ─compara hash──▶│ Model User    │
└────────┘  { email, password }       └────────────┘ (bcrypt.compare)└──────────────┘
     ▲                                       │
     │     200 OK + JWT (token firmado)      │
     └───────────────────────────────────────┘

RUTA PROTEGIDA (próximo paso)
┌────────┐  GET /api/v1/algo   ┌───────────────────┐   valida token   ┌────────────┐
│ Cliente│ ───Authorization:──▶│ jwt.strategy.js /  │ ────ok/error────▶│ Controller │
└────────┘  Bearer <token>     │ middleware auth     │                 └────────────┘
```
